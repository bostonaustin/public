...
def get_help():
    ...
    print("     aieManager.py -c start -n [node_name|adm-te] (node name|adm + processors)\n")
    print("     aieManager.py -c stop -n [node_name|adm-te] (node name|adm + processors)\n")
...
-- REFACTOR stop_node function

    # ORIGINAL function WORKS OK nut missing status messages
    # aie.sendline('stop node %s' % n)
    # aie.expect('aie>')
    # results = aie.before
    # return results, aie

    # ORIGINAL STEP THRU EACH VALID NODE
    # elif n in nodes:
    #     for n in nodes:
    #         restart_node(aie, n)

    # works OK if no timeout ? -- keep testing
        # nodes, aie = get_status(aie)
        # if nodes[n] == "RUNNING":
        #     results, aie = manage_node(aie, "stop", n)
        #     running = True
        #     i = 0
        ...
        #         sys.stdout.flush()
        #         time.sleep(60)
        #     print(" [OK - %s has been stopped]" % n)
        # elif nodes[n] == "STOPPED":
        #     print("  [WARNING] node %s appears to be already STOPPED" % n)
        #     sys.exit(1)

    # try adding pexpect timeout here ... ?
        # try:
        #     index = aie.expect(['aie>', pexpect.TIMEOUT])
        #     if index == 0:
        #         results = aie.before
        #         return results, aie
        #     elif index == 1:
        #         print("  [ERROR] - something went wrong with the pexpect.TIMEOUT piece")
        # except TIMEOUT:
        #     kill_node(aie, n)

    # TEST -- works but timeout fails -- not optimal
        # try:
        #     aie.sendline('stop node %s' % n)
        #     aie.expect('aie>')
        #     results = aie.before
