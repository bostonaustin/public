...
EOC
  ...
  if [[ ${ANSIBLE_HOSTS} == "" ]]; then
    echo "export ANSIBLE_HOSTS=/root/create-petclinc/ec2.py" >> ~/.bash_profile
  fi
  if [[ ${EC2_INI_PATH} == "" ]]; then
    echo "export EC2_INI_PATH=/root/create-petclinc/ec2.ini" >> ~/.bash_profile
  fi
...
if [ $? == 2 ]; then
  ...
  echo " "
  echo -e '  eval $(ssh-agent); ssh-add /root/.ssh/kp_se_eval.pem; ssh-add /media/psf/Home/Dropbox/create-petclinc/vagrant/files/ubuntu_vagrant; ssh-add -l '
else
  ...
  echo " "
  echo -e '          pkill ssh-agent; eval $(ssh-agent); ssh-add /root/.ssh/kp_se_eval.pem; ssh-add /media/psf/Home/Dropbox/create-petclinc/vagrant/files/ubuntu_vagrant; ssh-add -l '
fi
